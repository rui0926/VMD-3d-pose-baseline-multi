...
1. `同軸間引き角度` が聞かれるので、回転系ボーン間引き時の同軸回転角度(-180～180度の整数のみ)を指定する
    - 同軸上の回転が指定された角度範囲内であれば、キーフレームを間引く
    - 未指定の場合、デフォルトで「15」とする
1. `異軸間引き角度` が聞かれるので、回転系ボーン間引き時の異軸回転角度(-180～180度の整数のみ)を指定する
    - 異軸上の回転が指定された角度範囲内であれば、キーフレームを間引く
    - 未指定の場合、デフォルトで「3」とする
1. `センター移動間引き量`、`IK移動間引き量`、`同軸間引き角度`、`異軸間引き角度`すべてに0が指定された場合、間引かない
...
- ニコニコ動画の場合、コンテンツツリーへ動画\([sm33217872](http://www.nicovideo.jp/watch/sm33217872)\)を登録してください。
